...
def create_panorama(video: np.ndarray, video_shifts: np.ndarray=None) -> np.ndarray:
    """
    Stitches a video into a panorama using translation shifts.
    """
    ...
    if video_shifts:
        ...
        dx, dy, _ = get_video_shifts(video)

    # 2. Compute cumulative path (absolute positions)
    # The camera position at frame i is the sum of all previous shifts.
    # Note: If camera moves Right (+5), we place the next frame at +5.
    abs_x = np.cumsum(dx).astype(int)
    abs_y = np.cumsum(dy).astype(int)

    # 3. Calculate Bounding Box
    # We need to fit the extent of every frame placed at (abs_x, abs_y)
    # Extent of frame i is [abs_x[i] : abs_x[i]+w]
    min_x = np.min(abs_x)
    max_x = np.max(abs_x) + w
    min_y = np.min(abs_y)
    max_y = np.max(abs_y) + h
    ...
    # 4. Allocate Canvas
    # Check if video is grayscale (3D shape) or Color (4D shape)
    if video.ndim == 3:
        panorama = np.zeros((pano_h, pano_w), dtype=video.dtype)
    else:
        panorama = np.zeros((pano_h, pano_w, video.shape[3]), dtype=video.dtype)

    # 5. Paste Frames (Painter's Algorithm)
    # We iterate and paste frames. Later frames overwrite earlier ones.
    # (You could also implement averaging or median blending here)
    for i in range(num_frames):
        # Calculate offset coordinates (shifting local (0,0) to global canvas)
        # We subtract min_x/min_y to shift negative coordinates into valid positive indices
        y_start = abs_y[i] - min_y
        x_start = abs_x[i] - min_x

        y_end = y_start + h
        x_end = x_start + w

        # Paste the frame
        panorama[y_start:y_end, x_start:x_end] = video[i]
